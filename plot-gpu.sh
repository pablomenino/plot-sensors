...
echo "logging...";
rm plot_gpu_t_log.txt
nms=0
while [  $nms -lt $ntimes ];  do
        nvidia-settings -t -query  GPUTargetFanSpeed -q GPUCoreTemp | awk '{print $0}' | head -n 2 | tr "\n" "\t" >> plot_gpu_temp.txt
        let nms=nms+1
        sleep  $step
        now=$(date +"%d/%m/%Y %T")
#       echo $now
        echo -e "$(cat plot_gpu_temp.txt)""\t$now"  >> plot_gpu_t_log.txt
        rm plot_gpu_temp.txt
done
...
if [[ "$b" > "$day" ]]
then
        gnuplot -e "filename='plot-images/plot_gpu_$fcode'" plot_day_gpu
else
        gnuplot -e "filename='plot-images/plot_gpu_$fcode'" plot_time_gpu
fi
